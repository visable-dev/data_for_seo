defmodule DataForSeo.DataModel.GoogleSerpTranslatorTest do
  ...
  describe "serp/google/organic" do
    test "task-post" do
      assert %Task{result: nil} = translate_task_from_fixture(["serp", "google", "organic", "task-post"])

    end

    test "task-get-regular" do
      assert %Task{result: result} = translate_task_from_fixture(["serp", "google", "organic", "task-get-regular"])

      assert [%{datetime: ~U[2019-11-15 12:57:46Z],
    ...
    test "tasks-ready" do
      %Task{result: tasks} = translate_task_from_fixture(["serp", "google", "organic", "tasks-ready"])
