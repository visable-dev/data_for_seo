defmodule DataForSeo.Config do
  def current_scope do
    if Process.get(:_data_for_seo_auth, nil), do: :process, else: :global
  end
  ...
  def get(:global) do
    Application.get_env(:data_for_seo, :auth, nil)
  end

  def get(:process), do: Process.get(:_data_for_seo_auth, nil)

  @doc """
  Set Auth configuration values.
  """
  def set(value), do: set(current_scope(), value)
  def set(:global, value), do: Application.put_env(:data_for_seo, :auth, value)

  def set(:process, value) do
    Process.put(:_data_for_seo_auth, value)
    :ok
  end
